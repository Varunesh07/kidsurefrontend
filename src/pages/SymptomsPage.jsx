...
export default function SymptomsPage() {
  ...
  const handleAiContinue = () => {
    ...
    navigate(`/symptoms/results?aiText=${encodeURIComponent(aiText)}`);
  }
  ...
  return (
    <div className="flex flex-col h-full bg-l1 min-h-screen">
      ...
      <div className="flex-1 px-5 md:px-7 pb-24">
        ...
        {/* Toggle UI */}
        <div className="flex bg-l1 border border-l3 rounded-xl p-1 mb-6 mt-2 max-w-[400px]">
          <button 
            onClick={() => setMode('ai')}
            className={`flex-1 py-2 text-[13px] font-bold rounded-lg transition-all ${mode === 'ai' ? 'bg-white shadow-sm text-ink' : 'text-mid'}`}
          >
            Describe (AI)
          </button>
          <button 
            onClick={() => setMode('manual')}
            className={`flex-1 py-2 text-[13px] font-bold rounded-lg transition-all ${mode === 'manual' ? 'bg-white shadow-sm text-ink' : 'text-mid'}`}
          >
            Select from List
          </button>
        </div>
        ...
        {mode === 'ai' ? (
          <div className="bg-white border border-l3 rounded-[16px] p-5 shadow-sm">
            <h2 className="text-[15px] font-bold text-ink mb-1">Explain the situation</h2>
            <p className="text-[13px] text-mid mb-4">Our Medical AI will instantly map your description to the correct specialist.</p>
            <textarea 
              className="w-full bg-l1 border border-l3 rounded-xl p-4 text-[14px] text-ink outline-none focus:border-teal min-h-[140px] resize-none transition-colors"
              placeholder="e.g. My 5-year-old fell off the swings and her elbow is swelling rapidly..."
              value={aiText}
      ...
      {(mode === 'manual' && selectedIds.length > 0) || (mode === 'ai' && aiText.trim().length > 0) ? (
        <div className="fixed bottom-[70px] md:bottom-0 left-0 md:left-[230px] right-0 p-4 z-30 pointer-events-none">
           <div className="max-w-[400px] mx-auto pointer-events-auto">
             <button
               onClick={mode === 'ai' ? handleAiContinue : handleContinue}
             ...
             >
               <span className="text-[13px] font-bold">
                 {mode === 'ai' ? 'Analyze & Find Specialists' : `Find specialists (${selectedIds.length})`}
               </span>
